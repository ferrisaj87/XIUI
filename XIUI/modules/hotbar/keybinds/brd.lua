
-- Load and initialize the include file.keybinds_job = {}

keybinds_job['Base'] = {
     
  -- Hotbar #1
    {'battle 1 1', 'ma', 'Cure', 'stpc', 'Cure1'},
    {'battle 1 2', 'ma', 'Cure II', 'stpc', 'Cure2'},
    {'battle 1 3', 'ma', 'Cure III', 'stpc', 'Cure3'},
	-----------------------------------------------------------------
	{'battle 1 4', 'ma', 'Carnage Elegy', 't', 'Elegy2'},
    {'battle 1 4', 'ma', 'Battlefield Elegy', 't', 'Elegy1'},
	-----------------------------------------------------------------
    {'battle 1 5', 'ma', 'Regen', 'stpc', 'Regen'},
    {'battle 1 6', 'ma', 'Foe Lullaby', 't', 'Foe'},
	{'battle 1 7', 'ma', 'Horde Lullaby', 't', 'Horde'},
	{'battle 1 8', 'ma', 'Magic Finale', 't', 'Finale'},
	--{'battle 1 9', '', '', '', ''},
	--{'battle 1 10', '', '', '', ''},
	--{'battle 1 11', '', '', '', ''},
	{'battle 1 12', 'input', '//exec rdm/cookierest.txt', '', 'Cookie'},

    
  -- Hotbar #2 
	--{'battle 2 1', '', '', '', ''},
	--{'battle 2 2', '', '', '', ''},
	--{'battle 2 3', '', '', '', ''},
	--{'battle 2 4', '', '', '', ''},
	--{'battle 2 5', '', '', '', ''},
	--{'battle 2 6', '', '', '', ''},
	--{'battle 2 7', '', '', '', ''},
	--{'battle 2 8', '', '', '', ''},
	--{'battle 2 9', '', '', '', ''},
	--{'battle 2 10', '', '', '', ''},
	--{'battle 2 11', '', '', '', ''},
	--{'battle 2 12', '', '', '', ''},


  -- Hotbar #3
    {'battle 3 1', 'ma', 'Valor Minuet III', 'me', 'Min3'},
	{'battle 3 1', 'ma', 'Valor Minuet', 'me', 'Min1'},
	--------------------------------------------------------------------------------------
	{'battle 3 2', 'ma', 'Valor Minuet IV', 'me', 'Min4'},
	{'battle 3 2', 'ma', 'Valor Minuet II', 'me', 'Min2'},
	--------------------------------------------------------------------------------------
	{'battle 3 3', 'ma', 'Sword Madrigal', 'me', 'Mad1'},
	{'battle 3 4', 'ma', 'Blade Madrigal', 'me', 'Mad2'},
	{'battle 3 5', 'ma', 'Advancing March', 'me', 'March1'},
	{'battle 3 6', 'ma', 'Victory March', 'me', 'March2'},
	--{'battle 3 7', '', '', '', ''},
	--{'battle 3 8', '', '', '', ''},
	--{'battle 3 9', '', '', '', ''},
	--{'battle 3 10', '', '', '', ''},
	--{'battle 3 11', '', '', '', ''},
	--{'battle 3 12', '', '', '', ''},
  
  -- Hotbar #4
	 {'battle 4 1', 'ma', 'Blink', 'me', 'Blink'},
  	 {'battle 4 2', 'ma', 'Stoneskin', 'me', 'Stoneski'},
  	 {'battle 4 3', 'ma', "Mage's Ballad", 'me', 'Ballad'},
 	 {'battle 4 4', 'ma', "Mage's Ballad II", 'me', 'Ballad'},
 	 {'battle 4 5', 'ma', "Army's Paeon", 'me', 'Paeon'},
	 {'battle 4 6', 'ma', "Army's Paeon II", 'me', 'Paeon2'},
  	

	--{'battle 4 7', '', '', '', ''},
	--{'battle 4 8', '', '', '', ''},
	--{'battle 4 9', '', '', '', ''},
	--{'battle 4 10', '', '', '', ''},
	--{'battle 4 11', '', '', '', ''},
	--{'battle 4 12', '', '', '', ''},

  -- Hotbar #5 
	--{'battle 5 1', '', '', '', ''},
	--{'battle 5 2', '', '', '', ''},
	--{'battle 5 3', '', '', '', ''},
	--{'battle 5 4', '', '', '', ''},
	--{'battle 5 5', '', '', '', ''},
	--{'battle 5 6', '', '', '', ''},
	--{'battle 5 7', '', '', '', ''},
	--{'battle 5 8', '', '', '', ''},
	--{'battle 5 9', '', '', '', ''},
	--{'battle 5 10', '', '', '', ''},
	--{'battle 5 11', '', '', '', ''},
	--{'battle 5 12', '', '', '', ''},

  -- Hotbar #6
	--{'battle 6 1', '', '', '', ''},
	--{'battle 6 2', '', '', '', ''},
	--{'battle 6 3', '', '', '', ''},
	--{'battle 6 4', '', '', '', ''},
	--{'battle 6 5', '', '', '', ''},
	--{'battle 6 6', '', '', '', ''},
	--{'battle 6 7', '', '', '', ''},
	--{'battle 6 8', '', '', '', ''},
	--{'battle 6 9', '', '', '', ''},
	--{'battle 6 10', '', '', '', ''},
	--{'battle 6 11', '', '', '', ''},
	--{'battle 6 12', '', '', '', ''},
	
}



keybinds_job['WHM'] = {
 
	-- Hotbar #1 (1-0)
  
	-- Hotbar #2 (ALT 1-0)
	{'battle 2 1', 'ma', 'Poisona', 'stpc', 'Poisona'},
	{'battle 2 2', 'ma', 'Paralyna', 'stpc', 'Paralyna'},
	{'battle 2 3', 'ma', 'Blindna', 'stpc', 'Blindna'},
	{'battle 2 4', 'ma', 'Silena', 'stpc', 'Silena'},
	{'battle 2 5', 'ma', 'Cursna', 'stpc', 'Cursna'},
	{'battle 2 6', 'ma', 'Viruna', 'stpc', 'Viruna'},
	{'battle 2 7', 'ma', 'Erase', 'stpc', 'Erase'},
	{'battle 2 8', 'ja', 'Divine Seal', 'me', 'Div.Seal'},
	{'battle 2 9', 'ma', 'Curaga', 'stpc', 'Curaga2'},
	{'battle 2 10', 'ma', 'Reraise', 'me', 'Reraise'},
	{'battle 2 11', 'ma', 'Protectra', 'stpc', 'Protectra2'},
	{'battle 2 12', 'ma', 'Shellra', 'stpc', 'Shellra2'},
	
  
	-- Hotbar #3 (CTRL 1-0)
	
	
	-- Hotbar #4 (SHIFT 1-0)
   
	
   
  }

keybinds_job['NIN'] = {
	{'battle 2 2', 'ma', 'Utsusemi: Ichi', 'me', 'Ichi'},
	{'battle 2 1', 'ma', 'Utsusemi: Ni', 'me', 'Ni'},
	
  }

  keybinds_job['Sword'] = {
	-- Hotbar #1 
	{'battle 1 9', 'ws', 'Fast Blade', 't', 'Fast'},
	{'battle 1 10', 'ws', 'Flat Blade', 't', 'Flat'},
	{'battle 1 11', 'ws', 'Spirits Within', 't', 'Spirits'}, 
	
	
  }

  keybinds_job['Dagger'] = {
	-- Hotbar #1 
	{'battle 1 9', 'ws', 'Wasp Sting', 't', 'Wasp'},
	{'battle 1 10', 'ws', 'Viper Bite', 't', 'Viper'},
	{'battle 1 11', 'ws', 'Evisceration', 't', 'Eviscer'}, 
	
	
  }



return keybinds_job
