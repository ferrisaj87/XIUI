...
function M.DrawSettings()
    ...
    components.DrawCheckbox('Display Settings Button', 'vanaTimeShowSettingsBtn');
    imgui.ShowHelp('Show a gear icon on the VanaTime module that opens these settings directly.');

    -- ── Scaling & Layout ──────────────────────────────────────────────────────
    if components.CollapsingSection('Scaling & Layout##vt', true) then
        components.DrawSlider('Scale##vt', 'vanaTimeScale', 0.5, 2.0, '%.2f', nil, 1.0);
        imgui.ShowHelp('Global scale for the entire VanaTime window.  Double right-click to reset.');

        components.DrawSlider('Font Size##vt', 'vanaTimeFontSize', 8, 24, '%d', nil, 12);
        imgui.ShowHelp('Font size for clock and moon phase text (scaled by Scale above).  Double right-click to reset.');

        components.DrawSlider('Icon Size##vt', 'vanaTimeIconSize', 16, 64, '%d', nil, 28);
    ...
    if components.CollapsingSection('Display Options##vt', true) then
        components.DrawCheckbox("VT Time matches Element Color", 'vanaTimeVTElementColor');
        ...
        if gConfig.vanaTimeShowPastFuture ~= false then
            imgui.Indent(16);
            components.DrawSlider('Past / Future Opacity', 'vanaTimePastFutureOpacity', 0.0, 1.0, '%.2f', nil, 0.35);
            imgui.ShowHelp('Opacity of past and future day columns (0 = invisible, 1 = fully opaque).  Double right-click to reset.');
            imgui.Unindent(16);
        end

        components.DrawCheckbox('Disable Elemental Icons On Days', 'vanaTimePlainDayIcons');
        imgui.ShowHelp('Replace the elemental icon on each day column with a solid fill of the element\'s color.');

        components.DrawCheckbox('Show Weakness Badge', 'vanaTimeShowWeaknessBadge');
    ...
    if components.CollapsingSection('Weather Tab##vt', true) then
        ...
        components.DrawCheckbox('Enable Weather Tab', 'vanaTimeShowWeather');
        ...
        if gConfig.vanaTimeShowWeather ~= false then
            ...
            imgui.Text('Side');
            imgui.SameLine(0, 8);
            components.Combo('##vanaTimeWeatherSide', gConfig, 'vanaTimeWeatherSide',
                SIDE_LABELS, SIDE_VALUES, 'right', nil, 90);
            local weatherSide = gConfig.vanaTimeWeatherSide or 'right';
            if weatherSide == 'above' or weatherSide == 'below' then
                imgui.SameLine(0, 4);
                components.Combo('##vanaTimeWeatherAlign', gConfig, 'vanaTimeWeatherAlign',
                    ALIGN_LABELS, ALIGN_VALUES, 'left', nil, 72);
            ...
            components.DrawCheckbox('Hide Non-Elemental Weather##weather', 'vanaTimeWeatherHideNonElemental');
            ...
            if not hideNonElem then
                components.DrawCheckbox('Adjust Size for Elemental Weather##weather', 'vanaTimeWeatherAdjustElemental');
            ...
            components.DrawCheckbox('Individual Scaling##weather', 'vanaTimeWeatherCustomScale');
            ...
            if gConfig.vanaTimeWeatherCustomScale then
                imgui.Indent(16);
                if not hideNonElem then
                    local baseLbl = gConfig.vanaTimeWeatherAdjustElemental
                        and 'Non-Elemental Icon Size##weatherIcon'
                        or  'Icon Size##weatherIcon';
                    components.DrawSlider(baseLbl, 'vanaTimeWeatherIconSize', 16, 64, '%d', nil, 28);
                ...
                if not hideNonElem and gConfig.vanaTimeWeatherAdjustElemental then
                    components.DrawSlider('Elemental Icon Size##weatherElem', 'vanaTimeWeatherElementalIconSize', 16, 64, '%d', nil, 42);
                    if imgui.IsItemActive() then
                ...
                elseif hideNonElem then
                    ...
                    components.DrawSlider('Icon Size##weatherElemOnly', 'vanaTimeWeatherElementalIconSize', 16, 64, '%d', nil, 42);
                    if imgui.IsItemActive() then
                        _G.XIUI_weatherElementalPreview = true;
                    end
    ...
    if components.CollapsingSection('Tooltips##vt', true) then
        components.DrawCheckbox('Enable Tooltips', 'vanaTimeEnableTooltips');
        imgui.ShowHelp('Master toggle for all hover tooltips in the VanaTime module.');
        if gConfig.vanaTimeEnableTooltips ~= false then
            imgui.Indent(16);
            components.DrawCheckbox('VT##tipvt',      'vanaTimeTipVT');
            imgui.SameLine(0, 8);
            components.DrawCheckbox('LT##tiplt',      'vanaTimeTipLT');
            imgui.SameLine(0, 8);
            components.DrawCheckbox('TOD##tiptod',    'vanaTimeTipTod');
            imgui.SameLine(0, 8);
            components.DrawCheckbox('Weather##tipwx', 'vanaTimeTipWeather');
            imgui.Spacing();
            ...
            if gConfig.vanaTimeTooltipFenrir or gConfig.vanaTimeTooltipSeleneBow then
                imgui.Indent(16);
                components.Combo('Day Column Direction##vt', gConfig, 'vanaTimeTooltipDirection',
                    TT_DIR_LABELS, TT_DIR_VALUES, 'above');
                imgui.ShowHelp('Whether the day column tooltip appears above or below the VanaTime window.');
                imgui.Unindent(16);
            end
            imgui.Unindent(16);
        end
    ...
    if components.CollapsingSection('Timers##vt', true) then
        components.DrawCheckbox('Enable Timers', 'vanaTimeShowTimers');
        ...
        if gConfig.vanaTimeShowTimers ~= false then
            imgui.Indent(16);
            components.Combo('Timers Side##vt', gConfig, 'vanaTimeTimerSide',
                TT_DIR_LABELS, TT_DIR_VALUES, 'above');
            imgui.ShowHelp('Whether the timer panel appears above or below the VanaTime window.');
            components.DrawSlider('Timers Font Size##vt', 'vanaTimeTimersFontSize', 8, 24, '%d', nil, 12);
            imgui.ShowHelp('Font size of text inside the timers panel.  Double right-click to reset.');
            imgui.Spacing();
            components.DrawCheckbox('Auto-Close on Outside Click', 'vanaTimeTimersAutoCloseClick');
            imgui.ShowHelp('Automatically close the Timers panel when you click anywhere outside of it.');
            components.DrawCheckbox('Auto-Close on Inactivity', 'vanaTimeTimersAutoCloseIdle');
            ...
            if gConfig.vanaTimeTimersAutoCloseIdle then
                imgui.Indent(16);
                components.DrawSlider('Idle Seconds##vt_timers', 'vanaTimeTimersAutoCloseIdleSec', 1, 60, '%d', nil, 5);
    ...
    if components.CollapsingSection('Background##vt', false) then
        ...
        components.DrawSlider('Background Scale##vt', 'vanaTimeBgScale', 0.1, 3.0, '%.2f', nil, 1.0);
        imgui.ShowHelp('Scale of the background texture (Window themes only).  Double right-click to reset.');

        components.DrawSlider('Background Opacity##vt', 'vanaTimeBackgroundOpacity', 0.0, 1.0, '%.2f', nil, 0.85);
        ...
        components.DrawSlider('Border Scale##vt', 'vanaTimeBorderScale', 0.1, 3.0, '%.2f', nil, 1.0);
        imgui.ShowHelp('Scale of window border pieces (Window themes only).  Double right-click to reset.');

        components.DrawSlider('Border Opacity##vt', 'vanaTimeBorderOpacity', 0.0, 1.0, '%.2f', nil, 1.0);
