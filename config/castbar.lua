...
-- AlwaysClamp matches the slider policy used in config/components.lua; without it, double-click-
-- to-type accepts out-of-range values (e.g. negative or >1.0) that yield broken cast-speed math.
local function DrawFastCastSlider(jobName, jobIndex)
    local value = { gConfig.castBarFastCast[jobIndex] };
    if (imgui.SliderFloat('Fast Cast - ' .. jobName, value, 0.00, 1.00, '%.2f', ImGuiSliderFlags_AlwaysClamp)) then
...
function M.DrawSettings()
    ...
    if components.CollapsingSection('Fast Cast Settings##castBar') then
        ...
        if gConfig.castBarFastCastEnabled then
            ...
            if (imgui.SliderFloat('Fast Cast - RDM SubJob', castBarFCRDMSJ, 0.00, 1.00, '%.2f', ImGuiSliderFlags_AlwaysClamp)) then
            ...
            if (imgui.SliderFloat('WHM Cure Speed', castBarFCWHMCureSpeed, 0.00, 1.00, '%.2f', ImGuiSliderFlags_AlwaysClamp)) then
            ...
            if (imgui.SliderFloat('BRD Sing Speed', castBarFCBRDSingSpeed, 0.00, 1.00, '%.2f', ImGuiSliderFlags_AlwaysClamp)) then
